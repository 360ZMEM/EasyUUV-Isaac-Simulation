...
class EasyUUVEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: EasyUUVEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.PID_args = torch.zeros(self.num_envs, self.cfg.num_actions, 3, device=self.device)
        self.old_actions = torch.zeros(self.num_envs, self.cfg.num_actions, device=self.device)
        self.actions_i = torch.zeros(self.num_envs, self.cfg.num_actions, device=self.device)
        self.action_lim = torch.tensor([0.3, 0.3, 0.3, 0.3]).reshape(1, 4).to(self.device)

        self.log_MSE = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
